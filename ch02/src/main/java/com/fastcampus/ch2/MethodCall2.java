...
public class MethodCall2 {
	public static void main(String[] args) throws Exception{
		...
		//3. Model을 생성, Model은 인터페이스, BindingAwareModelMap이 구현제
		Model model = new BindingAwareModelMap(); //BindingAwareModelMap 실제 Model의 구현체
		System.out.println("[before] model="+model);
		
		//4. main메서드를 호출
		// String viewName = obj.main(2021, 10, 1, model); 아래 줄과 동일
		// 위에는 일반함수 호출, 아래는 Reflection API 호출
		//invoke를 사용하는데 (String)메서드.invoke(함수명, 매개변수)
